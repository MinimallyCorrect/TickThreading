...
public class TickThreading {
	...
	public void preInit(FMLPreInitializationEvent event) {
		...
		Property tickThreadsProperty = config.get(GENERAL, "tickThreads", tickThreads);
		tickThreadsProperty.comment = "number of threads to use to tick. 0 = automatic";
		Property enableChunkTickThreadingProperty = config.get(GENERAL, "enableChunkTickThreading", enableChunkTickThreading);
		enableChunkTickThreadingProperty.comment = "Whether chunk ticks should be threaded";
		Property enableWorldTickThreadingProperty = config.get(GENERAL, "enableWorldTickThreading", enableWorldTickThreading);
		enableWorldTickThreadingProperty.comment = "Whether world ticks should be threaded";
		Property regionSizeProperty = config.get(GENERAL, "regionSize", regionSize);
		regionSizeProperty.comment = "width/length of tick regions, specified in blocks.";
		Property variableTickRateProperty = config.get(GENERAL, "variableRegionTickRate", variableTickRate);
		...
		Property ticksCommandName = config.get(GENERAL, "ticksCommandName", TicksCommand.name);
		ticksCommandName.comment = "Name of the command to be used for performance stats. Defaults to ticks.";
		Property tpsCommandName = config.get(GENERAL, "tpsCommandName", TPSCommand.name);
		tpsCommandName.comment = "Name of the command to be used for TPS reports.";
		Property profileCommandName = config.get(GENERAL, "profileCommandName", ProfileCommand.name);
		profileCommandName.comment = "Name of the command to be used for profiling reports.";
		Property dumpCommandName = config.get(GENERAL, "dumpCommandName", DumpCommand.name);
		dumpCommandName.comment = "Name of the command to be used for profiling reports.";
		Property exitOnDeadlockProperty = config.get(GENERAL, "exitOnDeadlock", exitOnDeadlock);
		exitOnDeadlockProperty.comment = "If the server should shut down when a deadlock is detected";
		Property requireOpForTicksCommandProperty = config.get(GENERAL, "requireOpsForTicksCommand", requireOpForTicksCommand);
		requireOpForTicksCommandProperty.comment = "If a player must be opped to use /ticks";
		Property requireOpForProfileCommandProperty = config.get(GENERAL, "requireOpsForProfileCommand", requireOpForProfileCommand);
		requireOpForProfileCommandProperty.comment = "If a player must be opped to use /profile";
		Property requireOpForDumpCommandProperty = config.get(GENERAL, "requireOpsForDumpCommand", requireOpForDumpCommand);
		requireOpForDumpCommandProperty.comment = "If a player must be opped to use /dump";
		Property saveIntervalProperty = config.get(GENERAL, "saveInterval", saveInterval);
		saveIntervalProperty.comment = "Time between auto-saves, in ticks.";
		Property deadLockTimeProperty = config.get(GENERAL, "deadLockTime", deadLockTime);
		deadLockTimeProperty.comment = "The time(seconds) of being frozen which will trigger the DeadLockDetector.";
		Property shouldLoadSpawnProperty = config.get(GENERAL, "shouldLoadSpawn", shouldLoadSpawn);
		shouldLoadSpawnProperty.comment = "Whether chunks within 200 blocks of world spawn points should always be loaded.";
		Property enableFastMobSpawningProperty = config.get(GENERAL, "enableFastMobSpawning", enableFastMobSpawning);
		...
		Property disabledFastMobSpawningDimensionsProperty = config.get(GENERAL, "disableFastMobSpawningDimensions", new int[]{-1});
		disabledFastMobSpawningDimensionsProperty.comment = "List of dimensions not to enable fast spawning in.";
		Property waitForEntityTickProperty = config.get(GENERAL, "waitForEntityTickCompletion", waitForEntityTickCompletion);
		...
		Property chunkCacheSizeProperty = config.get(GENERAL, "chunkCacheSize", chunkCacheSize);
		...
		Property chunkGCIntervalProperty = config.get(GENERAL, "chunkGCInterval", chunkGCInterval);
		chunkGCIntervalProperty.comment = "Interval between chunk garbage collections in ticks";
		Property targetTPSProperty = config.get(GENERAL, "targetTPS", targetTPS);
		targetTPSProperty.comment = "TPS the server should try to run at.";
		Property concurrentNetworkTicksProperty = config.get(GENERAL, "concurrentNetworkTicks", concurrentNetworkTicks);
